...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        while (ros::ok() && nh.param("run", true))
        {
            ...
            if (ang_rbt > M_PI) ang_rbt -= 2 * M_PI;
            if (ang_rbt < - M_PI) ang_rbt += 2 * M_PI;
            double rt = lin_vel / ang_vel;
            if (abs(ang_vel) > straight_thresh) {
                pos_rbt.x = pos_rbt.x + rt * (sin(ang_rbt) - sin(ang_rbt_prev));
            ...
            if (verbose)
            {
                ...
                ROS_INFO("TMOTION_ERR: Pos(%7.3f, %7.3f) Ang_ERR(%6.3f)", pos_rbt.x - msg_odom.pose.pose.position.x,
                        pos_rbt.y - msg_odom.pose.pose.position.y, ang_rbt - atan2(siny_cosp, cosy_cosp));
            }
